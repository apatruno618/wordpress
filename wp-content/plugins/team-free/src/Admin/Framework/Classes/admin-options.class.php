...
if ( ! class_exists( 'TEAMFW_Options' ) ) {
	class TEAMFW_Options extends TEAMFW_Abstract {
		...
		public function add_options_html() {
			...
			// echo '<div class="spf-form-result spf-form-success ' . esc_attr( $notice_class ) . '">' . $notice_text . '</div>';

			// echo ( $this->args['show_form_warning'] ) ? '<div class="spf-form-result spf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'team-free' ) .'</div>' : '';
			...
			if ( $show_buttons ) {
				...
				echo '<h1 class="export-import"><img src="' . SPT_PLUGIN_ROOT . 'src/Admin/img/import-export.svg">' . $this->args['framework_title'] . '</h1>';
			}
			echo '</div>';
			...
			// echo ( $this->args['show_form_warning'] ) ? '<div class="spf-form-result spf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'spf' ) .'</div>' : '';
			...
			if ( $show_buttons ) {
				...
				echo '<input type="submit" name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary spf-top-save spf-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save', 'team-free' ) . '" data-save="' . esc_html__( 'Saving...', 'team-free' ) . '">';
			...
			if ( $has_nav ) {
				...
				foreach ( $this->pre_tabs as $tab ) {
					...
					if ( ! empty( $tab['subs'] ) ) {
						...
						echo '<a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '" class="spf-arrow">' . $tab_icon . $tab['title'] . $tab_error . '</a>';
						...
						echo '<li class="spf-tab-item"><a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '">' . $tab_icon . $tab['title'] . $tab_error . '</a></li>';
			...
			foreach ( $this->pre_sections as $section ) {
				...
				echo ( $has_nav ) ? '<div class="spf-section-title"><h3>' . $section_icon . $section_title . '</h3></div>' : '';
			...
			echo ( $has_nav && $nav_type === 'normal' ) ? '<div class="spf-nav-background"></div>' : '';
