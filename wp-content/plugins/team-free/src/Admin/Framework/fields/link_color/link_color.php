 ...
 *
 */
if ( ! class_exists( 'TEAMFW_Field_link_color' ) ) {
  class TEAMFW_Field_link_color extends TEAMFW_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'team-free' ),
        'hover'    => esc_html__( 'Hover', 'team-free' ),
        'active'   => esc_html__( 'Active', 'team-free' ),
        'visited'  => esc_html__( 'Visited', 'team-free' ),
        'focus'    => esc_html__( 'Focus', 'team-free' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="spf--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="spf-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
