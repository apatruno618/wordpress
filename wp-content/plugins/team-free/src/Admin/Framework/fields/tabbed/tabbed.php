...
if ( ! class_exists( 'TEAMFW_Field_tabbed' ) ) {
	class TEAMFW_Field_tabbed extends TEAMFW_Fields {
		...
		public function render() {
			...
			echo '<div class="spf-tabbed-nav" data-depend-id="' . esc_attr( $this->field['id'] ) . '">';
			foreach ( $this->field['tabs'] as $key => $tab ) {

				$tabbed_icon   = ( ! empty( $tab['icon'] ) ) ? '<i class="spf--icon ' . esc_attr( $tab['icon'] ) . '"></i>' : '';
			...
			echo '<div class="spf-tabbed-contents">';
			foreach ( $this->field['tabs'] as $key => $tab ) {

				$tabbed_hidden = ( ! empty( $key ) ) ? ' hidden' : '';

				echo '<div class="spf-tabbed-content' . esc_attr( $tabbed_hidden ) . '">';
