...
class Admin {
	...
	public function sptp_member_post_type() {
		...
		$labels     = apply_filters(
			...
			array(
				'name'                  => wp_sprintf( ( esc_html( 'All %s', 'team-free' ) ), $this->sptp_member_plural_name ),
				'singular_name'         => wp_sprintf( ( esc_html( '%1$1s %2$2s', 'team-free' ) ), $this->sptp_team_name, $this->sptp_member_singular_name ),
				'add_new'               => wp_sprintf( esc_html( 'Add New', 'team-free' ), $this->sptp_member_singular_name ),
				'add_new_item'          => wp_sprintf( esc_html( 'Add New %s', 'team-free' ), $this->sptp_member_singular_name ),
				'edit_item'             => wp_sprintf( esc_html( 'Edit %s', 'team-free' ), $this->sptp_member_singular_name ),
				'new_item'              => wp_sprintf( esc_html( 'New %s', 'team-free' ), $this->sptp_member_singular_name ),
				'all_items'             => wp_sprintf( esc_html( 'All %s', 'team-free' ), $this->sptp_member_plural_name ),
				'view_item'             => wp_sprintf( esc_html( 'View %s', 'team-free' ), $this->sptp_member_singular_name ),
				'search_items'          => wp_sprintf( esc_html( 'Search %s', 'team-free' ), $this->sptp_member_singular_name ),
				'not_found'             => wp_sprintf( esc_html( 'No %1$1s %2$2s Found', 'team-free' ), $this->sptp_team_name, $this->sptp_member_singular_name ),
				'not_found_in_trash'    => wp_sprintf( esc_html( 'No %1$1s %2$2s Found in Trash', 'team-free' ), $this->sptp_team_name, $this->sptp_member_singular_name ),
				'parent_item_colon'     => null,
				'menu_name'             => __( 'WP Team', 'team-free' ),
				'featured_image'        => wp_sprintf( esc_html( '%s Image', 'team-free' ), $this->sptp_member_singular_name ),
				'set_featured_image'    => wp_sprintf( esc_html( 'Set %s image', 'team-free' ), $this->sptp_member_singular_name ),
				'remove_featured_image' => wp_sprintf( esc_html( 'Remove %s image', 'team-free' ), $this->sptp_member_singular_name ),
			)
		);
	...
	public function sptp_review_text( $text ) {
		...
		if ( ( 'sptp_member' === get_post_type() ) || ( 'sptp_member_page_team_help' === $screen->id ) || ( 'sptp_member_page_team_settings' === $screen->id ) || ( 'sptp_generator' === get_post_type() ) || ( 'edit-sptp_group' === $screen->id ) ) {
			...
			$text = sprintf( __( 'If you like <strong>WP Team</strong>, please leave us a <a href="%s" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating. Your Review is very important to us as it helps us to grow more. ', 'team-free' ), $url );
		}
